<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>

<%@ attribute name="producers" required="true" type="java.util.Collection" %>
<%@ attribute name="searchForm" required="true" type="ru.belyaev.shop.form.SearchForm" %>

<div class="panel-heading">Producers filters</div>
<div class="panel-body producers">
    <label><input type="checkbox" id="allProducers"> All</label>
    <c:forEach var="pr" items="${producers}">
        <div class="form-group">
            <div class="checkbox">
                <label><input type="checkbox" name="producer" value="${pr.id}" class="search-option" ${searchForm.producers.contains(pr.id) ? 'checked' : ''} >${pr.name} (${pr.productCount}) </label>
            </div>
        </div>
    </c:forEach>
</div>