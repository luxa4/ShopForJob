...
<form class="search" action="/search">
	<div id="findProducts" class="panel panel-success collapse">
		...
		<div id="searchOptions" class="collapse ${searchForm.categoriesNotEmpty or searchForm.producersNotEmpty ? 'in' : '' }">
			...
			<ishop:producer-filter searchForm="${searchForm}" producers="${PRODUCER_LIST}" />
...
<div id="productCatalog" class="panel panel-success collapse">
	...
	<div class="list-group">
		...
		<a href="/products${c.url}" class="list-group-item ${c.url == selectedCategoryUrl ? 'active' : ''}"> <span class="badge">${c.productCount} </span> ${c.name} </a>
		</c:forEach>
	</div>
</div>
<!-- /Categories -->